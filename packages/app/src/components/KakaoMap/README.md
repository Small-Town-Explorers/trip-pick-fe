...
```tsx
import { KakaoLocationPickerMap, KakaoRouteMap } from '@components/KakaoMap';
...
주소로 단일 위치를 찾거나 지도 클릭 위치를 주소로 변환할 때는 아래처럼 사용합니다. 같은 주소를 다시 검색할 수 있도록 요청마다 `requestId`를 증가시켜 전달합니다.

```tsx
<KakaoLocationPickerMap
  initialCenterAddress="전라남도 강진군"
  addressSearchRequest={{ address: '전라남도 강진군 강진읍 탐진로 111', requestId: 1 }}
  selectedCoordinate={{ lat: 34.642, lng: 126.767 }}
...
- `initialCenterAddress`: 선택 좌표가 없을 때 지도의 초기 중심으로 사용할 지역 주소입니다. 마커나 선택값은 만들지 않습니다.
- `addressSearchRequest`: 카카오 주소 검색(`addressSearch`)을 실행해 좌표와 마커를 갱신합니다.
- `selectedCoordinate`: 기존에 선택한 위치를 지도에 표시합니다.
- `onLocationSelect`: 주소 검색 성공 또는 지도 클릭 후 역지오코딩(`coord2Address`) 성공 시 호출합니다.
