...
interface MyTripFolderScreenProps { folderId: string; }
interface CourseItem { id: string; title: string; image: unknown; upcoming: boolean; }
  ...
  return (
    <Screen testID={`my-trip-folder-${folderId}`}>
      <Header>
        ...
        <TitleRow><Title>폴더명</Title><Count>({courses.length})</Count></TitleRow>
        <Spacer />
      </Header>
      <Scroll contentContainerStyle={contentStyle}>
        {upcomingCourses.length > 0 && <Section><SubTitle>다가오는 여행</SubTitle>{upcomingCourses.map(renderCourse)}</Section>}
        {pastCourses.length > 0 && <Section><SubTitle>지난 여행</SubTitle>{pastCourses.map(renderCourse)}</Section>}
      </Scroll>
      <Action>
      ...
      >
        <DeleteMessage>&apos;{deleteTarget?.title ?? ''}&apos; 코스를{`\n`}삭제하시겠습니까?</DeleteMessage>
      </ConfirmModal>
    </Screen>
  );
}
...
const Header = styled.View({ height: 64, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20, borderBottomWidth: 1, borderBottomColor: colors.gray[100] });
const Back = styled.Pressable({ width: 24, height: 24 });
const TitleRow = styled.View({ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 });
const Title = styled.Text({ ...typography.heading2.medium, color: colors.gray[1000] });
const Count = styled.Text({ ...typography.body1.regular, color: colors.gray[400] });
...
const Course = styled.Pressable<{ active: boolean }>(({ active }) => ({ width: '100%', minHeight: 119, padding: 16, flexDirection: 'row', alignItems: 'flex-start', gap: 16, zIndex: active ? 20 : 0, backgroundColor: '#FFFFFF', borderRadius: 12, ...Platform.select({ web: { boxShadow: '0 0 40px rgba(8,25,29,0.1)' }, android: { elevation: 5 } }) }));
const CourseImage = styled.Image({ width: 87, height: 87, borderRadius: 8 });
const CourseBody = styled.View({ flex: 1, height: 87, gap: 8 });
const CourseHeader = styled.View({ width: '100%', flexDirection: 'row', alignItems: 'center', gap: 8 });
const CourseTitle = styled.Text({ flex: 1, ...typography.body2.semibold, color: colors.gray[1000] });
const MenuButton = styled.Pressable({ padding: 2 });
const Date = styled.Text({ ...typography.body3.medium, color: colors.gray[600] });
const Chip = styled.Text({ alignSelf: 'flex-start', paddingHorizontal: 10, paddingVertical: 5, ...typography.caption2.medium, color: colors.primary[700], backgroundColor: colors.gray[50], borderRadius: 9999 });
const DeleteButton = styled.Pressable({ position: 'absolute', right: -8, top: 54, zIndex: 30, height: 44, paddingHorizontal: 14, flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#FFFFFF', borderRadius: 10, ...Platform.select({ web: { boxShadow: '0 4px 20px rgba(8,25,29,0.12)' }, android: { elevation: 8 } }) });
const DeleteText = styled.Text({ ...typography.body2.medium, color: colors.gray[600] });
const Action = styled.View({ position: 'absolute', right: 0, bottom: 0, left: 0, paddingHorizontal: 20, paddingVertical: 24, backgroundColor: '#FFFFFF', zIndex: 40 });
const DeleteMessage = styled.Text({ ...typography.body2.regular, color: colors.gray[600], textAlign: 'center' });
